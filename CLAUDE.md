...
│   ├── MCA_DB_v1_10_20260403.xml    # Current versioned XML snapshot
│   ├── MCA_DB_v1_10_20260403.sql    # Current SQL dump (generated by xml2sql.py)
...
2. Import data: `mysql MCA < database/MCA_DB_v1_10_20260403.sql`
...
5. To regenerate the SQL dump from XML: `python3 database/xml2sql.py database/MCA_DB_v1_10_20260403.xml`
...
2. **Regenerate SQL** — run `python3 database/xml2sql.py database/MCA_DB_v[X]_[Y0]_[DATE].xml` and confirm output counts (passports, associations, UNCERTAIN skipped).
...
5. **Commit** — stage XML, SQL, `web/data/MCA_DB_latest.xml`, CLAUDE.md, and any other changed files. Commit message format: `v[X].[Y0]: [brief description]`.
6. **Create GitHub Release** — go to `https://github.com/Snyder-Institute/MCA/releases/new`; tag `v[X].[Y0]`; attach three release assets (gzip all SQL files — do **not** commit `.gz` files to the repo):
   - `MCA_create_database.sql.gz` — run `gzip -k database/MCA_create_database.sql`
   - `MCA_DB_v[X]_[Y0]_[DATE].xml` — raw XML snapshot (no compression)
   - `MCA_DB_v[X]_[Y0]_[DATE].sql.gz` — run `gzip -k database/MCA_DB_v[X]_[Y0]_[DATE].sql`
   Write release notes.
